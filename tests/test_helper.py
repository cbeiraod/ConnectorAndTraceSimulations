...
class TestFDTDMesher1DEquivalence:
    """
    Targeted equivalence tests to ensure the new _relax_grid_engine based aliases
    perfectly match the physical and topological outputs of the legacy methods.
    """

    @pytest.mark.parametrize("fixed", [
        [0.0, 1.0, 2.1],           # Standard unforced or slightly forced
        [0.0, 0.1, 10.0],          # Heavy grading cascade
        [0.0, 10.0, 10.1],         # Requires topological stagnation insertion
        [-5.0, -0.3, 0.3, 5.0]     # Symmetric behavior
    ])
    def test_jacobi_first_order_equivalence(self, fixed):
        """Proves _iterative_relaxation_jacobi is identical to the legacy _iterative_relaxation."""
        max_res = 2.0
        ratio = 1.5

        # Run Legacy Method
        mesher_old = FDTDMesher1D(fixed, [], max_res=max_res, ratio=ratio)
        mesh_old = mesher_old._iterative_relaxation(
            max_iterations=50000,
            relaxation_factor=0.2
        )

        # Run New Refactored Engine
        mesher_new = FDTDMesher1D(fixed, [], max_res=max_res, ratio=ratio)
        mesh_new = mesher_new._iterative_relaxation_jacobi(
        ...
        # 1. Verify topological parity (same number of stagnation point insertions)
        assert len(mesh_old) == len(mesh_new), "Meshes diverged in topological point insertion."

        # 2. Verify physical coordinate parity
        assert mesh_old == pytest.approx(mesh_new, abs=1e-12), "Meshes diverged in coordinate relaxation."
    ...
    def test_alternatinggaussseidel_first_order_equivalence(self, fixed):
        """Proves _iterative_relaxation_alternatinggaussseidel matches legacy _iterative_relaxation_fast."""
        max_res = 2.0
        ratio = 1.5

        mesher_old = FDTDMesher1D(fixed, [], max_res=max_res, ratio=ratio)
        mesh_old = mesher_old._iterative_relaxation_fast(
            max_iterations=50000,
            relaxation_factor=0.3
        )

        mesher_new = FDTDMesher1D(fixed, [], max_res=max_res, ratio=ratio)
        mesh_new = mesher_new._iterative_relaxation_alternatinggaussseidel(
            update_type="first_order",
            lr_mode="uniform",
            damping_mode="uniform",
            max_iterations=50000,
    ...
    def test_jacobi_momentum_equivalence(self, fixed):
        """Proves _iterative_relaxation_jacobi(momentum) matches legacy _iterative_relaxation_momentum."""
        max_res = 2.0
        ratio = 1.5
    ...
    def test_alternatinggaussseidel_momentum_equivalence(self, fixed):
        """Proves _iterative_relaxation_alternatinggaussseidel(momentum) matches legacy _iterative_relaxation_fast_momentum."""
        max_res = 2.0
        ratio = 1.5
